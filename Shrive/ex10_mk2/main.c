	...
	)
{
	...
	for( j = 0; j < num_edges; j++ )
	{
		...
		sorted_weights[index] = edge_weights[j];
	...
	unsigned int* predecessor = NULL;
	unsigned int* end = NULL;

#ifdef THREADS
	#pragma omp parallel for default( none ) shared( tree, obj_value, is_prime, terminals, num_terminals, max_num_start_terminals, first_neighbours_index, num_neighbours, num_nodes, num_edges, sorted_weights, sorted_heads ) private( j, k, temp_tree, end, predecessor ) num_threads( THREADS )
#endif
		for( j = 0; j < max_num_start_terminals; j++ )	
		{
			
			temp_tree = calloc( num_edges, sizeof( unsigned int ) );
			predecessor = malloc( num_nodes * sizeof( unsigned int ) );
			end = malloc( num_terminals * sizeof( unsigned int ) );
			assert( temp_tree );
			assert( predecessor );
			assert( end );
			...
			terminals_connected = get_steiner_tree( is_prime, terminals, num_terminals, num_nodes, num_edges, first_neighbours_index, num_neighbours, sorted_weights, sorted_heads, temp_tree, predecessor, source );
			...
			// could only test connectivity of best tree as this opperation is expensive
			// however other result will have been lost...
			unsigned int tree_connected = 0;
			if( temp_obj_value < obj_value && terminals_connected == num_terminals )
				tree_connected = is_tree_valid( predecessor, terminals, end, num_nodes, num_terminals, source );

			#pragma omp critical ( objective_value )
				if( temp_obj_value < obj_value && tree_connected )
				{
