...
bool checker::isConnected(vector<Edge>& edges, int& startTerminal) { // function to check if graph is a tree.
  int n = utils::get_Nodes(edges).back(); 
  n++;
  vector<vector<int>> graph = utils::build_adjList(n, edges);
  bool visited[n] = {false}; // all node are by default unvisited.
  ...
  cout << "Solution is a tree." << endl; // otherwise, if all the checks pass, the graph is a tree.
  return true;
}
